...
namespace :learn_to_rank do
  ...
  task :generate_training_dataset, [:judgements_filepath, :svm_dir] do |_, args|
    ...
    csv = args.judgements_filepath || "tmp/autogenerated_relevancy_judgements_4.csv"
    svm_dir = args.svm_dir || "tmp/ltr_data"
    FileUtils.mkdir_p svm_dir

    puts "Loading judgements"

    judgements_data = Relevancy::LoadJudgements.from_csv(csv)

    puts "Embedding features"
    ...
    File.open("#{svm_dir}/train.txt", "a") do |train|
      File.open("#{svm_dir}/validate.txt", "a") do |validate|
        File.open("#{svm_dir}/test.txt", "a") do |test|
  ...
  namespace :reranker do
    ...
    task :train, [:svm_dir, :model_dir] do |_, args|
      ...
      model_dir = args.model_dir || "tmp/libsvm_200k"
      svm_dir = args.svm_dir || "tmp/ltr_data_query_features_1"
